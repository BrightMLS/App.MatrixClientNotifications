...
if(Test-Path "$rootDir\App.MatrixClientNotifications.exe" -PathType Leaf){
    Remove-Item -path "$rootDir\App.MatrixClientNotifications.exe"
}
...
switch ( $env:DEPLOYMENT_GROUP_NAME )
{
    ...
    "aue1t1z1cdg${deploymentGroupAppName}releasedeploy" {
        Rename-Item -path "$rootDir\app.test.config" -NewName App.MatrixClientNotifications.exe -Force
    ...
    "aue1p1z1cdg${deploymentGroupAppName}releasedeploy" {
        Rename-Item -path "$rootDir\app.prod.config" -NewName App.MatrixClientNotifications.exe -Force
